...
var games map[string]*GameOfLife

// GameOfLife Struct representing a single instance of Game of Life.
...
// NewGameOfLife newGame is a constructor for Game object.
func NewGameOfLife(width int, height int, cell int, color string, backgroundColor string, interval int64, nextRefresh int64) GameOfLife {
	//prepare the board
...
func update(id string) {
	updated := NewGameOfLife(games[id].width, games[id].height, games[id].cell, games[id].color, games[id].backgroundColor, games[id].interval, games[id].nextRefresh)
...
func jsStartGameOfLife(this js.Value, args []js.Value) interface{} {
	...
	game := NewGameOfLife(width, height, cell, "#eee", "#555", int64(interval*1000000), 0)
	games[id] = &game
...
func main() {
	games = make(map[string]*GameOfLife)
